; ( function (window) {
  ...
  const pinyinMap = {
    ...
    jia: "家加价假佳架甲嘉贾驾嫁夹稼钾挟拮迦伽颊浃枷戛荚痂茄颉镓笳珈岬胛袈郏葭袷瘕铗跏蛱恝哿",
  ...
  const pinyinEntries = Object.entries(pinyinMap)
  const duoyinMap = {}

  pinyinEntries.forEach(([py, charList]) => {
    [...charList].forEach(char => {
      pinyinEntries.forEach(([_py, _charList]) => {
        if (py !== _py && _charList.includes(char)) {
          if (!duoyinMap[char]) duoyinMap[char] = []
          if (!duoyinMap[char].includes(_py)) {
            duoyinMap[char].push(_py)
          }
        }
      })
    })
  ...
  const allDuoyinStr = Object.keys(duoyinMap).join('')

  const getArrangement = list => {
    const result = []
    const results = []
    const arrange = (list, index) => {
      for (var i = 0; i < list[index].length; i++) {
        result[index] = list[index][i]
        if (index != list.length - 1) {
          arrange(list, index + 1)
        } else {
          results.push([...result])
        }
      }
  ...
  window.getDuoyinHandledStrListGroup = str => {
    ...
    charList.forEach((char, charIndex) => {
      if (allDuoyinStr.includes(char)) {
        duoyinList.push({
          char,
          charIndex,
          yinList: duoyinMap[char],
        })
      }
    })

    if (duoyinList.length) {
      // generate template
      duoyinList.forEach(({ charIndex }) => {
  ...
  window.getCharPinyin = char => {
    const entry = pinyinEntries.find(([py, charList]) => charList.includes(char))
    return entry ? entry[0] : char
  }

  window.getStringPinyin = (str, onlyFirst) => {
    return Array
      .from(str)
      .map(s => {
        const charPinyin = getCharPinyin(s)
        return onlyFirst ? charPinyin[0] : charPinyin
      })
      .join('')
  }
