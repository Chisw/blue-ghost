; (function (window) {

  const {
    chrome,
    location,
  } = window

  window.addEventListener('load', () => {

    chrome.runtime.sendMessage({
      method: 'getLocalStorage',
      key: 'BLUE_GHOST_CONFIG',
    }, res => {
      if (res.data.moPageList.includes(location.host)) {

        const replaceIcon = 'data:image/png;base64,'
        
        const link = document.createElement('link')
        link.type = 'image/x-icon'
        link.rel = 'shortcut icon'

        document.getElementsByTagName('head')[0].appendChild(link)

        document.querySelectorAll('link[rel*="icon"]').forEach(link => link.href = replaceIcon)

        document.title = `MoPage - BlueGhost - ${document.title}`

        const style = document.createElement('style')

        style.innerHTML = `
          html {
            opacity: 0;
          }
          html:hover {
            opacity: 1;
            transition: opacity 1s;
          }
        `
        document.body.appendChild(style)
      }
    })

  })
}(window));