...
window.getCharPinyin = (char, onlyFirst) => {
  const entry = Object.entries(pinyinMap).find(([py, charList]) => charList.includes(char))
  return entry
    ? onlyFirst
      ? entry[0][0]
      : entry[0]
...
window.getStringPinyin = (str, onlyFirst) => {
  return Array.from(str).map(s => getCharPinyin(s, onlyFirst)).join('')
}
